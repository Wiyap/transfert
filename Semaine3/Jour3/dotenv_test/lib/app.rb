...
#login_twitter.update("Merci pour votre travail #{journalist_sample[i]} <3! #bonjourmonde @the_hacking_pro")

#end

#login_twitter.search("#bonjour_monde -rt", result_type: "recent").take(1).collect do |tweet|
...
login_twitter_streaming.filter(track: hashtag.join(",")).user do |object|
  case object
  when case.is_a?(Twitter::Tweet)
    puts "It's a tweet!"
    login_twitter.follow(object.user)
  end
end

# topics = ["#macron","courgette"]
# login_twitter_streaming.filter(track: topics.join(",")) do |object|
